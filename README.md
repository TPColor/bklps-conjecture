...
The conjecture asserts that if `G` is a finite simple `2`-connected graph of order `n ≥ 10` not isomorphic to `K_n`, `K_n^2`, nor `K_n^{n−2}`, then the Szeged–Wiener gap of `G` is `η(G) ≥ 2n`. This result is formalized in `BKLPS.lean` and is proved with our own notation as our Theorem 6. The formalization for the definitions in the papers are found in `Definitions.lean`.
